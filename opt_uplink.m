function[R_cf_opt_user] = opt_uplink(L1,K,tau,Pp,Pu,Phii_cf,etak_opt,PhiPhi,Num,Betak)
%% Calculate UL Rate base on optimal power control of pilot sequences
etak = mean(etak_opt)*ones(K,1);
%-------------------------------------------------------------------------
...
cvx_quiet true
   ...
   cvx_begin %sdp
      ...
      subject to
        for k=1:K
            Te1(:,k)'*x(:,1) + [Te2(1:(k-1),k); Te2((k+1):K,k) ]'*[x(1:(k-1),1); x((k+1):K,1)] + L1*sum(Gammaa(:,k)) <= (1/tnext)*L1^2*Pu*(sum(Gammaa(:,k)))^2*x(k,1) ;
        end              
        for k=1:K
            x(k,1)>=0;
            x(k,1)<=1;
  ...
  mu1k = zeros(K,1);
            for k=1:K
                mu1k(k,1) = L1^2*Pu*(sum(Gammaa(:,k)))^2*x_cf_opt(k,1)/( Te1(:,k)'*x_cf_opt(:,1) + [Te2(1:(k-1),k); Te2((k+1):K,k) ]'*[x_cf_opt(1:(k-1),1); x_cf_opt((k+1):K,1)] + L1*sum(Gammaa(:,k)) );
